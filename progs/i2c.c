...
bool write_burst(uint8_t addr, const uint8_t* data, size_t len)
{
    ...
    /*
     * Нулевая длина — считаем успешным no-op.
     * Шину не трогаем.
     */
    if (len == 0u) {
        return true;
     ...
     * Адрес + бит W=0
     */
    if (!i2c_write_byte(addr_w | 0u)) {
        return i2c_fail_with_stop_if_possible();
    }

    /*
     * Последовательная запись всех байт в рамках одной транзакции
     */
    for (i = 0u; i < len; ++i) {
...
bool read_burst(uint8_t addr, uint8_t* data, size_t len)
{
    ...
    /*
     * Читаем len байт:
     * - после всех промежуточных байт шлём ACK (send_nack = false)
     * - после последнего байта шлём NACK (send_nack = true)
     */
    for (i = 0u; i < len; ++i) {
        bool last = (i == (len - 1u));
